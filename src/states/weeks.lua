Fnf3dsRatio = 240/720 -- 240 (3ds height) / 720(fnf game height) = 0.3333*
local weeks = {}
...
function weeks:enter()
    boyfriend = love.filesystem.load("assets/sprites/kero/bf.lua")()
    girlfriend = love.filesystem.load("assets/sprites/kero/gf.lua")()
...
function weeks:initUI()
    ...
    healthbar = graphics.newImage(love.graphics.newImage(graphics.imagePath("kero/healthBar")))
    healthbar.x = 160
    healthbar.y = 35
    ...
    for i = 1, 4 do
        ...
        enemyArrows[i].orientation = noteRotations[i]
        boyfriendArrows[i].orientation = noteRotations[i]
...
function weeks:generateNotes(chart)
    ...
    for _, section in ipairs(chart.notes) do
        ...
        for j, noteData in ipairs(section.sectionNotes) do
            ...
            noteObject.orientation = noteRotations[id]
...
function weeks:bottomDraw()
    love.graphics.push()
        ...
        love.graphics.rectangle("fill", 312, 28, -health * 152, 15)
        healthbar:draw()
    ...
    love.graphics.print("Score: " .. score .. "  |  Misses: " .. misses .. "  |  Combo: " .. combo, 10, 14)
